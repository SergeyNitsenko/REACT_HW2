...
export const Footer = () => {
    ...
    // const { title } = menuFooter.find(item => item.url === url)

    return (
        <>

            <div className={styles.container}>
                <Link to='#' className={styles.buttonReturn}><img src={arrow}></img></Link>
                <div className={styles.infoFooter}>
                    <Logo />
                    <span className={styles.rights}>© ООО СК «АПШЕРОН»
                ...
                <div  className={styles.footerLinks}>
                     {
                        menuFooter.map(item => <Link to='#' src={item.url} className={styles.footerLink} >{item.title}</Link>)
                    } 
